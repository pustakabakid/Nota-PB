...
export default function NotaPreview({
  ...
  // Auto-fit A4 canvas scale to fit viewport width AND height without scrollbar
  useEffect(() => {
    const calculateScale = () => {
      if (!containerRef.current) return;
      const availW = window.innerWidth - 24;
      const availH = window.innerHeight - (hidePaperSelector ? 95 : 140);
      
      const targetW = selectedPaper === 'A4' ? 680 : (selectedPaper === 'A5' ? 540 : (selectedPaper === '80mm' ? 390 : 300));
      const targetH = selectedPaper === 'A4' ? 940 : (selectedPaper === 'A5' ? 760 : 700);
  ...
  }, [selectedPaper, hidePaperSelector]);
  ...
  const handleCopyText = () => {
    ...
    if (onShowToast) {
      onShowToast('Teks ringkasan nota berhasil disalin!', 'success');
    }
  };

  const targetWidth = selectedPaper === 'A4' ? 680 : (selectedPaper === 'A5' ? 540 : (selectedPaper === '80mm' ? 390 : 300));
  const targetHeight = selectedPaper === 'A4' ? 940 : (selectedPaper === 'A5' ? 760 : 700);

  return (
    <section className="nota-preview-container" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        
        {!hidePaperSelector && (
          <div className="paper-selector-bar">
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)' }}>
              <i className="ri-layout-3-line"></i> Format Kertas:
            </span>
        ...
        {/* Printable Nota Canvas Container with Auto-Fit Scaling (Zero-Scroll) */}
        <div ref={containerRef} className="nota-autofit-container" style={{
          width: '100%',
          display: 'flex',
          ...
          <div style={{
            ...
            <div id="printableNota" ref={printableRef} className="nota-canvas" data-paper={selectedPaper}>
              ...
              {/* Two Column Metadata (BILLING INFORMATION & PAYMENT METHOD) */}
        ...
        {/* Action Buttons Section (Fixed Bottom Bar matching Mockup Image) */}
        {!isSaved ? (
          ...
          <div className="fixed-action-footer">
            <CustomTooltip text="Cetak Langsung Ke Printer">
              <button type="button" className="btn-footer-action" onClick={handlePrint}>
                <i className="ri-printer-line"></i>
                <span>Cetak</span>
              </button>
            </CustomTooltip>
            <CustomTooltip text="Unduh Dokumen PDF">
              <button type="button" className="btn-footer-action" onClick={handleDownloadPdf}>
                <i className="ri-file-pdf-line"></i>
                <span>PDF</span>
              </button>
            </CustomTooltip>
            <CustomTooltip text="Kirim Nota via WhatsApp">
              <button type="button" className="btn-footer-action" onClick={handleShareWa}>
                <i className="ri-whatsapp-line"></i>
                <span>WA</span>
              </button>
            </CustomTooltip>
            <CustomTooltip text="Salin Teks Nota Ke Clipboard">
              <button type="button" className="btn-footer-action" onClick={handleCopyText}>
